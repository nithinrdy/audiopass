    ...
    pub fn new(
            ...
            .state_changed(|_stream, data, _old_state, new_state| {
                ...
                match new_state {
                    pipewire::stream::StreamState::Error(e) => {
                        data.sender.send(PipewireEvent::VirtualCaptureReady { state: Err(e) });
                    }
                    pipewire::stream::StreamState::Paused => {
                        data.sender.send(PipewireEvent::VirtualCaptureReady { state: Ok(()) });
                    }
        ...
        match stream.connect(
            ...
            &mut ([match Pod::from_bytes(get_serialized_vec_for_pod()?.deref()) {
                Some(p) => p,
                None => return Err("Failed to serialize libspa POD while creating virtual capture stream".to_string()),
            }]),
        ) {
            Ok(_) => {}
